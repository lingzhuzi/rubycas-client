...
After installing RubyCAS-Client as a plugin (see above), add the following to your app's <tt>config/initializer/cas_sso.rb</tt>
...
filter. For example:
  ...
  before_filter :sso_filter, :except => [ :unprotected_action, :another_unprotected_action ]
...
CAS authentication for all actions in a controller except the index action:
  ...
  class ExampleController < ApplicationController
    before_filter :sso_filter, :only => :index
    before_filter :gateway_filter, :except => :index
...
In your <tt>config/initializer/cas_sso.rb</tt>:
